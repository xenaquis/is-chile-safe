...
- [x] **DATA-01**: Scraper Python extrae frecuencias y casos policiales desde los endpoints CEAD (`get_estadisticas_delictuales*.php`) para las 346 comunas, 16 regiones y total país, todos los años disponibles (2005–presente) y las 7 familias de delito
...
- [x] **DATA-04**: La tasa por 100.000 habitantes es la métrica canónica del data contract; los rankings nacionales aplican filtro de población mínima (10.000 hab) y nunca ordenan por conteo absoluto

### Capa de noticias (NEWS)

- [x] **NEWS-01**: Pipeline ingesta RSS de fuentes confirmadas (BioBíoChile, Cooperativa Policial, La Tercera Nacional) con fallback por feed: un feed caído se salta y registra sin bloquear el pipeline
...
- [x] **NEWS-03**: Incidentes duplicados entre fuentes se deduplican antes de publicar
- [x] **NEWS-04**: `incidents/current.json` mantiene ventana móvil de 30 días con archivo mensual (`archive/YYYY-MM.json`)
...
- [x] **PAGES-02**: 16 páginas de región en ES y EN con agregados y ranking de sus comunas
- [x] **PAGES-03**: Páginas por tipo de delito (ES/EN) con mapa y ranking filtrados
...
- [x] **MON-01**: Integración AdSense activada tras la primera ola de indexación (metodología y legales ya indexadas)
