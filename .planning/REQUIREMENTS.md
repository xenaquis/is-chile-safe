...
- [ ] **TD-01**: Methodology EN+ES correctly state the national mean is an **unweighted** mean of non-low-population commune rates (a typical-commune figure), matching `data.ts` and the is-chile-safe FAQ — no "population-weighted" claim anywhere user-facing. *(MC-01, MC-07 — high)*
...
- [ ] **TD-06**: `map-payload-*.json` carries an explicit `partial_year` flag so the frontend can caveat partial years (e.g. 2025). *(DQ-MAP-PAYLOAD-2025-NO-PARTIAL-FLAG)*
