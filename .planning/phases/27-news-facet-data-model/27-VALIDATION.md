...
| 27-01-02 | 01 | 1 | FACET-01, FACET-03, FACET-04, FACET-05, FACET-06 | T-27-02, T-27-04, T-27-05 | `facets.mjs` proves region/CUT-length cross-check over all 346 communes, per-month union-of-IDs counts cross-checked against BOTH rendered locales, source-level H-2 escaping guard, family-set containment parsed live from Python source, byMonth completeness, TZ determinism, and stray-artifact absence (including gitignored `site/public/data/`); never writes to `data/`/`site/**` | validator (standalone Node script, 10 assertion blocks — **7 load-bearing**; see 27-VERIFICATION.md: assertion 1's count-sum and assertion 8's monotonic-containment comparisons are true by construction for every input, confirmed by a 20,000-input fuzz, and assertion 4 is unreachable behind assertion 3. Carried to Phase 28.) | `node site/scripts/validate/facets.mjs && echo GREEN` (corrected from the H1 no-op `; echo "EXIT:$?"` form) | ✅ (created by task) | ⬜ pending |
